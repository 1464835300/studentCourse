<template>
    <page-view class="main">
        <div class="navbar h-full">
            <el-menu unique-opened router :default-active="activeIndex" ref="menuRef" active-text-color="#ffd04b"
                text-color="#fff" background-color="#545c64" class="el-menu-vertical-demo" :collapse="isCollapse">
                    ...
                    <el-sub-menu :index="index1 + ''">
                        ...
                        <el-menu-item-group :title="row.text">
                            <template v-for="(item, index2) in row.items" :key="index2">
                                <el-menu-item @click="selectClick(item)" :index="item.url">{{ item.text
                                }}</el-menu-item>
                            </template>
                        </el-menu-item-group>
                    </el-sub-menu>
        ...
        <div class=" min-w-[400px] flex-1 overflow-auto flex flex-col">
            <div class="title-tab h-fit  relative">
                <div class="title-top shadow">
                    <div class=" mx-10 mt-10" v-if="isCollapse" @click="isCollapse = false">
                ...
                <div class="title-bottom  box-border pl-20 shadow">
                    <ul class="tabs list-none flex relative">
                        <template v-for="(row, index) in tabList">
                            <li class="item w-fit px-20 mx-0 transition-all cursor-pointer"
                                :class="{ isActive: tabsSelect === index }" @click='tabChange(index, row.url)'>
                                {{ row.text }}
                                <el-icon @click.stop="closeClick(index)" v-if="row.isClose" class="close-icon ml-5"
                                    :size="10" color=" #73767a">
                                    <CloseBold />
                                </el-icon>
                            </li>
                        </template>
                            ...
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        ...
                                        <el-dropdown-item @click="closeOther">关闭其他标签</el-dropdown-item>
                                    </el-dropdown-menu>
                                </template>
                            </el-dropdown>
                        </li>
...
const selectClick = (item) => {
    ...
    if (index === -1 || !index) {
        let obj = {
            text: item.text, url: item.url, isClose: true
        }
        tabList.push(obj);
